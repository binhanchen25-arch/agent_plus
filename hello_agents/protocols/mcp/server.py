...
def create_example_server() -> MCPServer:
    ...
    # ========== 资源（Resources）：提供可读取的上下文数据 ==========
    # 客户端通过 resources/list 发现资源，通过 resources/read 按 URI 读取内容
    def get_api_docs() -> str:
        """返回 API 文档，供 AI 了解可用接口"""
        return """
# Example Server API 文档

## 工具 (Tools)
- **calculator**: 计算数学表达式，如 "2+3*4"
- **greet**: 生成问候语，传入名字即可

## 资源 (Resources)
- file:///example-server/api-docs: 本 API 文档
- file:///example-server/config: 服务器配置信息
"""
...
"""
    ...
    server.add_resource(get_api_docs, uri="file:///example-server/api-docs")
    server.add_resource(get_server_config, uri="file:///example-server/config")
...
if __name__ == "__main__":
    ...
    print(f"📂 Resources: file:///example-server/api-docs, file:///example-server/config")
    print()
    server.run()
